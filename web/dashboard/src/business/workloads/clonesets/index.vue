<template>
  <layout-content header="Clonesets">
    <div style="float: left">
      <el-button v-has-permissions="{scope:'namespace',apiGroup:'apps',resource:'clonesets',verb:'create'}" type="primary" size="small" @click="yamlCreate">
          YAML
      </el-button>
      <el-button type="primary" size="small" @click="onCreate" v-has-permissions="{scope:'namespace',apiGroup:'apps',resource:'clonesets',verb:'create'}">
        {{ $t("commons.button.create") }}
      </el-button>
      <el-button type="primary" size="small" :disabled="selects.length===0" @click="onScale()" v-has-permissions="{ scope: 'namespace', apiGroup: 'apps', resource: 'clonesets', verb: 'update' }">
        {{ $t("commons.button.scale") }}
      </el-button>
      <el-button type="primary" size="small" :disabled="selects.length===0" @click="onDelete()" v-has-permissions="{scope:'namespace',apiGroup:'apps',resource:'clonesets',verb:'delete'}">
...
import { scaleCloneset, patchCloneset } from "@/api/clonesets"
...
export default {
      ...
      buttons: [
        {
          ...
          disabled: () => {
            return !checkPermissions({ scope: "namespace", apiGroup: "apps", resource: "clonesets", verb: "update" })
          },
        },
        {
        ...
        },
        {
          ...
          click: (row) => {
            downloadYaml(row.metadata.name + ".yml", getWorkLoadByName(this.clusterName, "clonesets", row.metadata.namespace, row.metadata.name))
          },
        },
        {
          ...
          disabled: () => {
            return !checkPermissions({ scope: "namespace", apiGroup: "apps", resource: "clonesets", verb: "delete" })
          },
        },
      ],
      loading: false,
  ...
  methods: {
    onCreate() {
      this.$router.push({ name: "ClonesetCreate", params: { operation: "create" }, query: { yamlShow: false } })
    },
    openDetail(row) {
      this.$router.push({
        name: "ClonesetDetail",
    ...
    yamlCreate() {
      this.$router.push({ name: "ClonesetCreateYaml", params: { operation: "create" }, query: { type: "clonesets" } })
    },
    onDelete(row) {
      this.$confirm(this.$t("commons.confirm_message.delete"), this.$t("commons.message_box.prompt"), {
        ...
        this.ps = []
        if (row) {
          this.ps.push(deleteWorkLoad(this.clusterName, "clonesets", row.metadata.namespace, row.metadata.name))
        } else {
          if (this.selects.length > 0) {
            for (const select of this.selects) {
              this.ps.push(deleteWorkLoad(this.clusterName, "clonesets", select.metadata.namespace, select.metadata.name))
            }
          }
        }
        if (this.ps.length !== 0) {
    ...
    onScaleSubmit() {
      ...
      for (const row of this.selects) {
        ...
        scaleCloneset(this.clusterName, row.metadata.namespace, row.metadata.name, row)
          .then(() => {
    ...
    onModifyVersionSubmit() {
      ...
      patchCloneset(this.clusterName, this.form.namespace, this.form.name, data)
        .then(() => {
    ...
    onRestart(row) {
      ...
      patchCloneset(this.clusterName, row.metadata.namespace, row.metadata.name, data)
        .then(() => {
          this.dialogModifyVersionVisible = false
          this.loading = true
          this.$message({
    ...
    search(resetPage) {
      ...
      listWorkLoads(this.clusterName, "clonesets", true, this.searchConfig.keywords, this.paginationConfig.currentPage, this.paginationConfig.pageSize)
        .then((res) => {
          this.data = res.items
